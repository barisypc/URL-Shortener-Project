...
def bulk_upload(
    ...
    results = []
    df = df.where(pd.notnull(df), None) #it replaces nan from excel etc. with None (Since excel/csv reads empty places as nan it creates problems when you turn it to Python.where )
